...
class User(AbstractUser):
    # custom user with rolebased access
    ROLE_CHOICES = (
        ('farmer', 'Farmer'),
        ('porter', 'Porter'),
        ('admin', 'Admin'),
...
class BaseModel(models.Model):
    # create an abstract class model with common timestamps
...
class FarmerProfile(BaseModel):
    # complete farmer profile or info needed
    ...
    farm_size_acres = models.DecimalField(max_digits=20,blank=True,null=True,decimal_places=2)
...
class PorterProfile(BaseModel):
    ...
    profile_image= models.ImageField(upload_to='porters/profiles/', null=True, blank=True)
...
class Payment(BaseModel):
    ...
    payment_methods=models.CharField(choices=METHOD_CHOICES,max_length=10)
